...
def reading_data_and_preparing_network(files_from_cl, gpu, itera, batch_size, train_path, labels_dic, mixing, nb_groups, nb_cl, save_path):
    dataset = utils_data.read_data_test(train_path, labels_dic, mixing, files_from_cl=files_from_cl, image_size=224)
    dataset_size = sum(1 for _ in dataset)
    if dataset_size == 0:
        raise ValueError("Dataset is empty.")
    image_train = dataset.map(lambda x, y, z: x)
    label_train = dataset.map(lambda x, y, z: y)
    file_string = dataset.map(lambda x, y, z: z)

    dataset = tf.data.Dataset.zip((image_train, label_train, file_string))
    ...
    with tf.variable_scope('ResNet18'):
        with tf.device('/gpu:'+gpu):
            scores         = utils_resnet.ResNet18(iterator.get_next()[0]-mean_img, phase='test',num_outputs=nb_cl*nb_groups)
            graph          = tf.get_default_graph()
            op_feature_map = graph.get_operation_by_name('ResNet18/pool_last/avg').outputs[0]
    
    label_batch_one_hot = tf.one_hot(iterator.get_next()[1], nb_groups*nb_cl)
    ...
    return inits,scores,label_batch_one_hot,loss_class,op_feature_map
